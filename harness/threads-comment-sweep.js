...
/**
 * Threads Comment Sweep Daemon
 * ============================
 * Reads business-goals.json for niches + ICP, then systematically comments
 * on relevant Threads posts — per niche + per feed, with daily caps.
 *
 * Limits (safe Threads usage):
 ...
 *
 * State persisted in threads-comment-sweep-state.json:
 *   - Daily comment count (resets at midnight)
 *   - Per-niche daily counts
 *   - Set of post URLs already commented on (last 7 days)
 *
 * Usage:
 *   node harness/threads-comment-sweep.js            # run as daemon
 *   node harness/threads-comment-sweep.js --once     # single sweep, then exit
 *   node harness/threads-comment-sweep.js --dry-run  # generate replies, don't post
 *   node harness/threads-comment-sweep.js --test     # preflight check only
 *   node harness/threads-comment-sweep.js --niche saas_growth  # single niche
 */
...
// ── Paths ────────────────────────────────────────────────────────────────────
const STATE_FILE = path.join(HARNESS_DIR, 'threads-comment-sweep-state.json');
const LOG_FILE   = path.join(HARNESS_DIR, 'threads-comment-sweep-log.ndjson');
...
// ── Config ───────────────────────────────────────────────────────────────────
const TH_COMMENTS_URL = process.env.THREADS_COMMENTS_URL || 'http://localhost:3004';
const TH_API_TOKEN    = process.env.THREADS_API_TOKEN || 'threads-local-dev-token';

const MAX_PER_NICHE   = parseInt(process.env.TH_MAX_PER_NICHE   || '5');
const MAX_PER_FEED    = parseInt(process.env.TH_MAX_PER_FEED    || '3');
const MAX_PER_RUN     = parseInt(process.env.TH_MAX_PER_RUN     || '20');
const DAILY_CAP       = parseInt(process.env.TH_DAILY_CAP       || '40');
const BASE_INTERVAL_MS = parseInt(process.env.TH_RUN_INTERVAL_MS || String(2 * 60 * 60 * 1000)); // 2h
...
function acquireLock(timeoutMs = 120_000) {
  ...
  try {
    ...
    fs.writeFileSync(SAFARI_LOCK, JSON.stringify({
      pid: process.pid, platform: 'threads',
...
async function thPost(path, body) {
  try {
    const res = await fetch(`${TH_COMMENTS_URL}${path}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${TH_API_TOKEN}`,
      },
...
async function thGet(path) {
  try {
    const res = await fetch(`${TH_COMMENTS_URL}${path}`, {
      headers: { 'Authorization': `Bearer ${TH_API_TOKEN}` },
...
async function preflight() {
  console.log('=== Threads Comment Sweep Preflight ===\n');

  const health = await thGet('/health');
  if (health.error || health.status !== 'ok') {
    console.log(`FAIL: threads-comments :3004 — ${health.error || health.status}`);
    return false;
  }
  console.log(`OK: threads-comments :3004 — ${health.status}`);
...
async function runSweep() {
  ...
  const sweepResult = await thPost('/api/threads/comment-sweep', {
    niches: nichesToRun,
...
async function main() {
  ...
  // Daemon mode
  log(`Threads Comment Sweep daemon started (PID ${process.pid})`);
...
main().catch(e => {
  console.error(`[th-sweep] Fatal: ${e.message}`);
  process.exit(1);
});
