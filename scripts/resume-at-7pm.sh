#!/bin/bash
# Resume BlogCanvas harness after rate limit reset at 7pm EST

echo "═══════════════════════════════════════════════════════════════"
echo "  BlogCanvas Harness - Waiting for Rate Limit Reset (7pm)"
echo "═══════════════════════════════════════════════════════════════"
echo ""
echo "  Current time: $(date)"
echo "  Rate limit resets at: 7:00 PM EST"
echo "  Progress: 32/40 features (80%)"
echo "  Remaining: 8 features"
echo ""

# Wait until 7:05pm (19:05)
TARGET_HOUR=19
TARGET_MIN=5

while true; do
    CURRENT_HOUR=$(date +%H)
    CURRENT_MIN=$(date +%M)
    
    if [ "$CURRENT_HOUR" -ge "$TARGET_HOUR" ]; then
        if [ "$CURRENT_HOUR" -gt "$TARGET_HOUR" ] || [ "$CURRENT_MIN" -ge "$TARGET_MIN" ]; then
            echo ""
            echo "✅ Rate limit should be reset! Starting harness..."
            break
        fi
    fi
    
    echo -ne "\r  Waiting... Current time: $(date +%H:%M:%S)  "
    sleep 60
done

# Start the harness
cd /Users/isaiahdupree/Documents/Software/autonomous-coding-dashboard
./scripts/run-blogcanvas-16hr.sh
