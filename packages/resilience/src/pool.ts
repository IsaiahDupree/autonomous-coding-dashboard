import { PoolConfig, PoolConfigSchema, PoolFactory } from './types';
...
export class ConnectionPool<T> {
  ...
  private readonly idle: PooledConnection<T>[] = [];
  private readonly active = new Set<T>();
  private readonly waiting: WaitingClient<T>[] = [];
  private draining = false;
  private initialized = false;

  constructor(factory: PoolFactory<T>, config: Partial<PoolConfig> = {}) {
    this.factory = factory;
    const parsed = PoolConfigSchema.parse(config);
    this.config = {
      min: parsed.min,
      max: parsed.max,
  ...
  async acquire(): Promise<T> {
    ...
    if (this.waiting.length >= this.config.maxWaitingClients) {
      throw new Error(
        `Pool waiting queue is full (${this.config.maxWaitingClients} clients waiting)`,
      );
    }
  ...
  private clearIdleTimer(pooled: PooledConnection<T>): void {
    if (pooled.idleTimer) {
