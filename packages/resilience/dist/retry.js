...
exports.createMetaRetryConfig = createMetaRetryConfig;
exports.createRemotionRetryConfig = createRemotionRetryConfig;
const types_1 = require("./types");
/**
 * Default predicate: retry on 429, 5xx, and network errors.
 */
function defaultRetryableErrors(error) {
 ...
 */
function calculateDelay(attempt, baseDelayMs, maxDelayMs, backoffMultiplier, jitterFactor) {
...
async function retry(fn, config) {
    const parsed = types_1.RetryConfigSchema.parse(config ?? {});
    const { maxRetries, baseDelayMs, maxDelayMs, backoffMultiplier, jitterFactor, retryableErrors, } = parsed;
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            const retryableErr = error;
            const retryAfterMs = parseRetryAfter(retryableErr.retryAfter);
            const delay = retryAfterMs ??
                calculateDelay(attempt, baseDelayMs, maxDelayMs, backoffMultiplier, jitterFactor);
            await sleep(delay);
        }
    }
    throw lastError;
 ...
 */
async function retryWithCircuitBreaker(fn, retryConfig, breaker) {
 ...
 */
function createMetaRetryConfig() {
    return {
        maxRetries: 5,
        baseDelayMs: 2000,
        maxDelayMs: 60000,
 ...
 */
function createRemotionRetryConfig() {
    return {
        maxRetries: 3,
        baseDelayMs: 5000,
        maxDelayMs: 60000,
        ...
        retryableErrors: (error) => {
            ...
            const msg = err.message.toLowerCase();
            return (msg.includes('render timed out') ||
