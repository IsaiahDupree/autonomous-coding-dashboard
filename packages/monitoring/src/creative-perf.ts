 ...
 * ```
 */
export class CreativePerformanceTracker {
  private readonly metricsHistory: Map<string, CreativeMetrics[]> = new Map();
  private weights: PerformanceWeights;
  ...
  recordMetrics(
    params: Omit<CreativeMetrics, 'ctr' | 'cvr' | 'roas' | 'engagementScore' | 'performanceScore' | 'timestamp'> & {
      ctr?: number;
      cvr?: number;
      roas?: number;
      engagementScore?: number;
    ...
    const metrics = CreativeMetricsSchema.parse({
      ...params,
  ...
  getTopCreatives(limit: number = 10): CreativeMetrics[] {
    const latest: CreativeMetrics[] = [];
  ...
  compareCreatives(
    creativeIdA: string,
    creativeIdB: string,
    metric: keyof Pick<CreativeMetrics, 'ctr' | 'cvr' | 'roas' | 'performanceScore' | 'engagementScore'> = 'performanceScore'
  ): ABComparison {
    ...
    if (!metricsA || !metricsB) {
      throw new Error(
        `Metrics not found for ${!metricsA ? creativeIdA : creativeIdB}`
      );
    }

    const valueA = metricsA[metric] as number;
    const valueB = metricsB[metric] as number;
    ...
    const confidenceLevel = Math.min(
      0.99,
      minSample >= 10000 ? 0.95 : minSample >= 1000 ? 0.85 : minSample >= 100 ? 0.70 : 0.50
    );
  ...
  private calculateEngagement(params: {
    ...
    if (params.impressions === 0) return 0;
  ...
  private calculatePerformanceScore(metrics: {
    ...
    return (
      normalizedCtr * this.weights.ctr +
