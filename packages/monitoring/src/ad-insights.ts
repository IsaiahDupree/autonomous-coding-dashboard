 ...
 * ```
 */
export class AdInsightsManager {
  private readonly campaigns: Map<string, CampaignPerformance[]> = new Map();

  /** Record or update campaign performance data. Derived metrics are auto-calculated. */
  recordCampaign(
    params: Omit<CampaignPerformance, 'roas' | 'ctr' | 'cpc' | 'cpa' | 'updatedAt'> & {
      roas?: number;
      ctr?: number;
      cpc?: number;
      cpa?: number;
  ...
  getSummary(
    ...
    return AdInsightsSummarySchema.parse({
