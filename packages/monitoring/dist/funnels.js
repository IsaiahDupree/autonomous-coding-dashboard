...
class FunnelEngine {
    ...
    /** Define a new funnel. */
    defineFunnel(params) {
        const funnel = types_1.FunnelDefinitionSchema.parse({
    ...
    trackEvent(params) {
        const event = types_1.FunnelEventSchema.parse({
        ...
        if (!validStepNames.includes(event.stepName)) {
            throw new Error(`Invalid step "${event.stepName}" for funnel "${event.funnelId}". Valid steps: ${validStepNames.join(', ')}`);
        }
        this.events.push(event);
        return event;
    }
    /** Generate a funnel report with conversion rates and drop-off analysis. */
    getReport(funnelId, period) {
        ...
        const stepUserSets = new Map();
        const stepEventCounts = new Map();
        const stepTimestamps = new Map(); // step -> userId -> timestamp
        ...
        for (const event of events) {
            ...
            if (userSet) {
                ...
                stepEventCounts.set(event.stepName, (stepEventCounts.get(event.stepName) ?? 0) + 1);
        ...
        const stepMetrics = sortedSteps.map((step, index) => {
        ...
        const reportPeriod = period ?? {
            start: events.length > 0
                ? events.reduce((min, e) => new Date(e.timestamp).getTime() < new Date(min).getTime()
                    ? e.timestamp
                    : min, events[0].timestamp)
                : new Date().toISOString(),
            end: events.length > 0
                ? events.reduce((max, e) => new Date(e.timestamp).getTime() > new Date(max).getTime()
                    ? e.timestamp
    ...
    getDropOffUsers(funnelId, fromStep, toStep) {
        const events = this.events.filter((e) => e.funnelId === funnelId);
        const fromUsers = new Set(events.filter((e) => e.stepName === fromStep).map((e) => e.userId));
        const toUsers = new Set(events.filter((e) => e.stepName === toStep).map((e) => e.userId));
