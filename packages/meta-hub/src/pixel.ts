...
// ---------------------------------------------------------------------------
// Internal HTTP helper (minimal, uses only built-in https)
// ---------------------------------------------------------------------------
...
export class MetaPixelClient {
  ...
  constructor(config: MetaPixelConfig, rateLimiterConfig?: RateLimiterConfig) {
    if (!config.pixelId) {
      throw new MetaValidationError('pixelId is required');
    }
    ...
    this.baseUrl = `https://graph.facebook.com/${DEFAULT_API_VERSION}`;
   ...
   */
  async sendEvents(events: CAPIEvent[]): Promise<{ eventsReceived: number }> {
    if (events.length === 0) {
    ...
    if (events.length > 1000) {
      throw new MetaValidationError(
        `Maximum 1000 events per batch, received ${events.length}`,
      );
    }

    // Validate all events
    const validatedEvents: CAPIEvent[] = events.map((evt, i) => {
      try {
        return CAPIEventSchema.parse(evt) as CAPIEvent;
      } catch (err: unknown) {
        throw new MetaValidationError(
          `Invalid event at index ${i}: ${(err as Error).message}`,
          `events[${i}]`,
    ...
    const response = await this.rateLimiter.executeWithRetry<EventsResponse>(
      bucketKey,
      async () => {
        return this.rateLimiter.submit<EventsResponse>('pixel', bucketKey, async () => {
          const res = await httpRequest('POST', url, body);

          this.rateLimiter.updateFromHeaders(bucketKey, res.headers);

          if (res.status >= 400) {
            const errorBody = res.body as MetaApiErrorResponse;
          ...
          return res.body as EventsResponse;
        });
      },
      isMetaRateLimitError,
    );
  ...
  hashUserData(data: UserData): HashedUserData {
    ...
    // Name fields
    if (data.fn !== undefined) hashed.fn = hashIfNeeded(data.fn);
    if (data.ln !== undefined) hashed.ln = hashIfNeeded(data.ln);

    // Demographics
    if (data.ge !== undefined) hashed.ge = hashIfNeeded(data.ge);
    if (data.db !== undefined) hashed.db = hashIfNeeded(data.db);

    // Location fields
    if (data.ct !== undefined) hashed.ct = hashIfNeeded(data.ct.replace(/\s/g, ''));
    if (data.st !== undefined) hashed.st = hashIfNeeded(data.st);
    if (data.zp !== undefined) hashed.zp = hashIfNeeded(data.zp);
    if (data.country !== undefined) hashed.country = hashIfNeeded(data.country);
    ...
    if (data.client_ip_address !== undefined) hashed.client_ip_address = data.client_ip_address;
    if (data.client_user_agent !== undefined) hashed.client_user_agent = data.client_user_agent;
    if (data.fbc !== undefined) hashed.fbc = data.fbc;
    if (data.fbp !== undefined) hashed.fbp = data.fbp;
    if (data.subscription_id !== undefined) hashed.subscription_id = data.subscription_id;
    if (data.lead_id !== undefined) hashed.lead_id = data.lead_id;
   ...
   */
  private hashUserDataForCapi(data: UserData): Record<string, unknown> {
