...
export class PCTVoiceService {
  private readonly voiceStorage: VoiceStorageService;
  private readonly remotionApiUrl: string;
  ...
  async getVoiceForAd(
    adId: string,
    preferences?: PCTVoicePreferences,
    ...
    const allVoices = await this.voiceStorage.listVoices(
      this.extractOrgFromAd(adId),
    );

    if (allVoices.length === 0) return null;

    if (!preferences) return allVoices[0];
   ...
   */
  async renderAdWithVoice(
    adInput: PCTAdRenderInput,
  ): Promise<PCTAdRenderResult> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Remotion render failed (${response.status}): ${body}`,
      );
    }

    const result = (await response.json()) as PCTAdRenderResult;
    return result;
  }
  ...
  async previewVoice(
    ...
    if (!response.ok) {
      const body = await response.text();
      throw new Error(
        `Voice preview failed (${response.status}): ${body}`,
      );
    }
   ...
   */
  async listAvailableVoices(orgId: string): Promise<VoiceReference[]> {
    const all = await this.voiceStorage.listVoices(orgId);
    const pctVoices = all.filter(
      (v) => v.tags.includes("pct") || v.tags.includes("ad"),
    );
   ...
   */
  private extractOrgFromAd(adId: string): string {
