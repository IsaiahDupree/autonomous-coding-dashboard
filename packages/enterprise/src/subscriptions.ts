...
// ─── BILL-005: Subscription Lifecycle ────────────────────────────────────────
...
export class SubscriptionService {
  ...
  calculateRevenueMetrics(): RevenueMetrics {
    ...
    const canceledThisPeriod = allSubs.filter(
      s => s.status === 'canceled' && s.canceledAt && s.canceledAt >= thirtyDaysAgo,
    ).length;

    const newThisPeriod = allSubs.filter(
      s => s.createdAt >= thirtyDaysAgo && s.status !== 'canceled',
    ).length;
  ...
  getRevenueTimeSeries(
    ...
    endTime: number,
    intervalMs: number,
  ): Array<{ timestamp: number; mrrCents: number; activeCount: number }> {
    const allSubs = Array.from(this.subscriptions.values());
    const series: Array<{ timestamp: number; mrrCents: number; activeCount: number }> = [];
