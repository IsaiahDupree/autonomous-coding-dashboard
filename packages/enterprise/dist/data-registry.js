...
exports.DataSubjectRegistry = void 0;
...
class DataSubjectRegistry {
    ...
    registerDataEntry(input) {
        const now = Date.now();
        let record = this.subjects.get(input.userId);
        if (!record) {
            record = types_1.DataSubjectRecordSchema.parse({
                userId: input.userId,
                email: input.email,
                dataEntries: [],
                registeredAt: now,
        ...
        const exists = record.dataEntries.some(e => e.tableName === input.tableName && e.recordId === input.recordId);
        if (!exists) {
    ...
    getEntriesByTable(userId, tableName) {
        const record = this.subjects.get(userId);
        if (!record)
            return [];
    ...
    removeDataEntry(userId, tableName, recordId) {
        const record = this.subjects.get(userId);
        if (!record)
            return false;
        const index = record.dataEntries.findIndex(e => e.tableName === tableName && e.recordId === recordId);
        if (index === -1)
            return false;
...
exports.DataSubjectRegistry = DataSubjectRegistry;
