...
function parseCronExpression(expr) {
    ...
    const intervalMatch = expr.match(/^every\s+(\d+)\s+(second|minute|hour|day|week)s?$/i);
    if (intervalMatch) {
    ...
    if (cronMinMatch) {
        return parseInt(cronMinMatch[1], 10) * 60000;
    }
...
class JobScheduler {
    ...
    startEntry(entry) {
        ...
        entry.timerId = setInterval(() => {
            if (job.running)
                return; // skip if previous run hasn't finished
            job.running = true;
